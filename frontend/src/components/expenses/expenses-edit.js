...
export class ExpensesEdit {
    ...
    async fillDataFromServer(id){
        const result = await CategoriesServices.getCategory(config.expCategory,id);
        if (result.error){
    ...
    async actionRecord() {
        ...
        if (this.operation === 'edit') {
            if (Object.keys(requestData).length === 0){
                ...
                result = await CategoriesServices.editCategory(config.expCategory,requestData);
            }
        }else{
            result=await CategoriesServices.addCategory(config.expCategory,requestData);
        }
