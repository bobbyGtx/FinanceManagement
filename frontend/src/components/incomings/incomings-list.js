...
export class IncomingsList {
    ...
    async getCardList() {
        const response = await CategoriesServices.getCategoriesList(config.incCategory);
        if (response.error) {
    ...
    showRecords(categoryList) {
        categoryList.forEach(categoryItem => {
            ...
            actionsElement.innerHTML = `
                <button onclick="document.location='#/incomings/edit?id=${categoryItem.id}&title=${categoryItem.title}'" type="button" class="btn btn-primary me-2">Редактировать</button>
        ...
        addCardBodyElement.innerHTML=`<a href="#/incomings/create"class="d-flex align-items-center justify-content-center h-100 w-100 p-0 fs-1">+</a>`;
        addCardElement.appendChild(addCardBodyElement);
        this.cardsContainerElement.appendChild(addCardElement);
    }
    deleteCategory(e){
        document.location='#/incomings/delete?id='+e;
    }
}